...
calcualte_da_lm<-function( list.df,
  ...
  for(d in 1:length(list.df)) {
    ...
    df<-data.frame( bin = vector(length = ncol(tmp.spec)),
                    ...
                    statistic = vector(length = ncol(tmp.spec)),
                    p.value = vector(length = ncol(tmp.spec)),
                    formula = vector(length = ncol(tmp.spec)))
    ...
    for(s in 1:ncol(tmp.spec)) {
      ...
      if(all(meta.tmp$SmplID != rownames(tmp.spec))) {
        ...
        if(class(x)[1] == "try-error"){
          ...
          mod<-l.mod[[d]][[s]]
          sumtidy<-broom::tidy(mod)
          print(model.type)
          
          df[s,"bin"]<-spec
          df[s,"estimate"]<-sumtidy[sumtidy$term == "IBDIBD",2]
          df[s,"std.error"]<-sumtidy[sumtidy$term == "IBDIBD",3]
          df[s,"statistic"]<-sumtidy[sumtidy$term == "IBDIBD",4]
          df[s,"p.value"]<-sumtidy[sumtidy$term == "IBDIBD",5]
          df[s,"formula"]<-model.type
